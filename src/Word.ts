function searchValueContainsReplacedWords(searchValue: string, replaceValue: string, replacedWords: Set<String>): boolean {
...
export default class Word {
  ...
  replace(searchValue: any, replaceValue: any, replaceReplacedWords: boolean = false): Word {
    if (!replaceReplacedWords && searchValueContainsReplacedWords(searchValue, replaceValue, this.replacedWords)) return this;
    const replacingWord: string = this.word.replace(searchValue, replaceValue).trim();
    const matchArray: RegExpMatchArray | null = this.word.match(searchValue);
    const replacedWords: string[] = (matchArray !== null) ? Array.from(matchArray).map(x => x.replace(x, replaceValue)) : [];
